...
class Nodes:
    ...
    def _check_new_emails(self,state:GraphState) -> str:
        """Checks if there are new emails to be processed"""
    ...
    def is_email_inbox_empty(self,state:GraphState) -> GraphState:
        """Checks if there there are no emails to process"""
        if len(state['emails']) == 0:
            print(Fore.RED + "No new emails" + Style.RESET_ALL)
            return "empty"
    ...
    def _is_email_inbox_empty(self,state:GraphState) -> GraphState:
        return state
    ...
    def construct_raq_queries(self,state:GraphState) -> GraphState:
    ...
    def retrieve_from_rag(self,state:GraphState) -> GraphState:
         ...
         for query in state["rag_queries"]:
             answer = self.agents.generate_rag_answer.invoke({"question": query})
             final_answer += answer + "\n"
    ...
    def write_draft_email(self,state:GraphState) -> GraphState:
            ...
            #Get messages history for the current email
    ...
    def must_rewrite(self,state:GraphState) -> str:
         ...
         if email_sendable:
              ...
              print(Fore.RED + "Email is not good, must rewrite!!!" + Style.RESET_ALL)
              return "rewrite"
